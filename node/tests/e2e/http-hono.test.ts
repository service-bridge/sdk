/**
 * Hono HTTP integration e2e — drives REAL HTTP requests through a Hono app
 * served by Bun.serve and instrumented via `attachHono`, then asserts that the
 * runtime persisted HTTP.HANDLE operations (SUCCESS + ERROR) for the
 * "http-test" service in Postgres.
 *
...
describe.skipIf(!enabled)("HTTP Hono integration e2e", () => {
	...
	test("Hono drives /hono with all methods + an error → SUCCESS and ERROR traces", async () => {
		const base = "/hono";
		const app = new Hono();
		...
		server = Bun.serve({ fetch: app.fetch, port }) as unknown as {
			stop(): void;
		};
		await sleep(300);
