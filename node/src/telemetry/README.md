...
| `OpHandle.start(ring, params)` | `OpHandle` | — | Enqueue START frame; auto-mint opId + inherit ALS trace. `params: StartOpParams`. Читает окружающий trace-scope, но НЕ открывает свой: код после вызова остаётся с прежним родителем, поэтому вложенные вызовы встают рядом с op'ом, а не под ним. Вложенность даёт `run` |
...
| `OpHandle.run(fn)` | `Promise<T>` | — | Выполняет `fn(handle)` внутри trace-scope этого op'а и закрывает op: `SUCCESS` при нормальном завершении, `ERROR` + текст исключения при throw (исключение пробрасывается дальше). Всё, до чего дотянется `fn` — `rpc.call`, `event.publish`, следующий `startOp` — становится дочерним. `fn` может закрыть op сам с более точным статусом (`TIMEOUT`, `ABANDONED`): `end` идемпотентен, автозакрытие тогда ничего не делает |
| `OpHandle.scope` | `TraceContext` (getter) | — | Контекст для дочерних операций: тот же `traceId`, `parentOpId` = `opId` этого op'а |
...
- **Scope открывает `run`, а не `start`**: op бывает двух видов. Внутренние (RPC.CALL, HTTP.HANDLE) сами решают, где начинается их scope: rpc-клиент считает `childContext` и оборачивает в него только сам вызов, http-плагины — только downstream chain, а не весь хук. Ручной пользовательский op хочет обратного: чтобы всё внутри блока стало дочерним. Разводить это флагом внутри `start` значило бы дуальный режим, поэтому `start` остаётся чистой эмиссией START-кадра, а вложенность — отдельный явный вызов `run(fn)`. Цена: `startOp()` без `run()` продолжает эмитить op рядом с его же вложенными вызовами — поэтому строка о том, что он не открывает scope, стоит и в JSDoc, и в таблице выше.
- **`run` использует `als.run`, а не `als.enterWith`**: `enterWith` ставит контекст на текущую async-ветку и не снимает его — после `end()` scope закончившегося op'а остался бы активным, и следующий соседний op получил бы родителем уже завершённого соседа. На нескольких параллельных `run` это ломается ещё раньше: ветки делят один async-scope, и последний `enterWith` перетирает родителя у всех остальных. `als.run` разворачивает контекст по выходу из callback'а, поэтому соседние и параллельные op'ы остаются siblings. `enterWith` в SDK остаётся ровно в одном месте — Fastify `preHandler`, где hook-API не даёт callback'а, охватывающего обработку запроса.
