...
Логический RPC-вызов в telemetry-модели — ровно одна `operations`-строка **RPC.CALL**, владелец = caller-SDK (ADR-0037..0042). Callee НЕ эмиттит op, runtime НЕ эмиттит op. Streaming, ретраи и proxy-факт укладываются в ту же единственную строку.
...
| `RpcAccessDeniedError` | class | — | Бросается из `RpcDomain.call()` при call-time запрете политики (`rpc.call`/`rpc.handle`, ADR-0014). Поля: `serviceName`, `methodName`, `reason`. |
...
| `idempotencyKey` | `string` | `""` (нет ключа) | Opt-in runtime-side dedup (ADR-0012). Пустая строка — wire-сигнал «нет ключа», runtime пропускает Claim/Save. Без ключа коды `INTERNAL`/`ABORTED`/`UNKNOWN` non-retryable. |
...
Retryable-коды: `UNAVAILABLE(14)`, `RESOURCE_EXHAUSTED(8)`, `DEADLINE_EXCEEDED(4)` — всегда; `INTERNAL(13)`, `ABORTED(10)`, `UNKNOWN(2)` — только при заданном `idempotencyKey` (ADR-0012). Ошибки без числового `.code` (application errors хендлера, schema errors) — non-retryable.
...
| Имя | Тип | Что делает |
|-----|-----|------------|
| `RpcClient` | class | Entry point исходящих вызовов. `call()`/`stream()` резолвят descriptor через `InstanceCache`, фильтруют кандидатов по `contractHash` (ADR-0005), пикают через `LoadBalancer`, кодируют payload `SchemaPair`, выбирают транспорт и эмиттят одну RPC.CALL op. |
...
| `LoadBalancer` | class | Power-of-Two-Choices (ADR-0009 §1) по inflight. `pick` исключает CB-OPEN и instances с health-hint моложе `HEALTH_HINT_TTL_MS`; `acquire(id)` → release-closure для `finally`; `inflightOf`. |
...
| `CircuitBreakerRegistry` | class | Sliding-window CB per `(serviceId, instanceId)` (ADR-0009 §2). Окно 10s × 10 buckets; OPEN при `total ≥ 10 && errorRate > 0.5` на 30s; HALF_OPEN допускает один probe. `canCall`/`recordSuccess`/`recordFailure`/`state`/`reset`. |
...
## Telemetry-модель (ADR-0037..0042, T-017, T-019)
...
- **SchemaRegistry на caller-стороне.** Caller владеет `.proto` target-сервиса. Несовпадение contract hash → fail-fast через `contractHash`-routing (ADR-0005).
...
- **Retry не применяется к streaming.** Mid-stream replay ре-доставил бы уже полученные чанки (ADR-0004).
- **Identity by UUID, имена display-only.** Идентификаторы внутри SDK и на проводе — `service_id` (UUID). `service_name` — только DX-поверхность и UI label (ADR-0014).
- **Direct-transport acceptance enforcement (ADR-0014 Gap 1).** `CallServer` извлекает SPIFFE-URI из verified peer-cert и проверяет локально по `PolicyEvaluation.acceptance`. Сломан accessor → fail-closed (`PERMISSION_DENIED`); peer без SPIFFE-URI = runtime-proxy (caller-gate уже отработал) → пропуск.
- **Одна RPC.CALL строка, владелец — caller-SDK (ADR-0037..0042).** Caller-SDK один видит и direct-, и proxy-путь, поэтому эмиттит единственную op. Ретраи — счётчик `attempt`; proxy-факт — `meta.via_proxy`.
