...
| `HttpRequestGuard` | `new (options?: HttpSecurityOptions)` | scanner block + 300 req/min/client | До telemetry отсекает поиск `.env`/VCS/PHP/WordPress и ограничивает частоту по клиенту. `X-Forwarded-For` учитывается только при явном `trustProxyHops`. |
...
- **Edge-защита до observability.** Адаптеры вызывают `HttpRequestGuard` до `startHttpOp`: scanner probes получают 404, превышение лимита — 429 + `Retry-After`, и оба класса не создают `operations`. Карта клиентов ограничена, forwarded IP fail-closed игнорируется без trusted proxy hops.
