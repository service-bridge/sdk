...
|------|-----|--------------|------------|
| `schema` | `SchemaSpec` | — (обязательно) | `.proto`-файл + имена сообщений (или `.schema.json`) для input и output; нужен для decode/encode и contract-hash (ADR 0005/0012) |
...
| `Handle.publishEvent(name, spec?)` | — | `spec` undefined | Объявляет published event (publisher-side). `spec` — `SchemaSpec`; async-загрузка в общий `pending[]`. Idempotent re-define по reference identity `spec`; иной spec — throws (ADR-0013) |
...
| `WatchStream.eventSubscriptionsSnapshot()` | `Map<string, EventSubscriptionDescriptor>` | — | Копия event-subs; ключ `"${serviceId}\|${pattern}"` (ADR-0014) |
| `WatchStream.outgoingCallsSnapshot()` | `Map<string, OutgoingCallDescriptor>` | — | Копия outgoing-calls; ключ `(caller, target, method, type)` (ADR-0014) |
| `WatchStream.policyEvaluation()` | `PolicyEvaluation \| null` | `null` | Последняя `PolicyEvaluation` (ADR-0014) |
...
- **EVENT не в `incomingMethods()`**: подписки едут только через `RegisterRequest.event_subscriptions`, dedup по pattern — серверный `event_subscriptions` имеет PRIMARY KEY `(subscriber_id, pattern)`. In-process fan-out на несколько хендлеров одного pattern делает SDK сам (ADR 0006).
