...
- **Состояние прогона — это JSON.** Вход прогона лежит под ключом `input`, выход каждого шага — под идентификатором этого шага. Отсюда и protobuf-типов на этой границе нет: шаги принимают и отдают обычные Go-значения.
